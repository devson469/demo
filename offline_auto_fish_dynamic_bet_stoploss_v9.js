(function () {
  ...
  function buildPanelObservationStatusHtml(status) {
    ...
    var targetId = Number(focusCandidate.tableEntityID || focusCandidate.symbolID || 0) || 0;
    var focusName = String(focusCandidate.name || "").trim();
    var candidateText = targetId > 0
      ? "target " + String(targetId) + (focusName ? " / " + focusName : "")
      : "暂无实时候选";
    var candidateScore = Number(focusCandidate.score || 0) || 0;
    var candidateHitProbability = Number(focusCandidate.hitProbability || 0) || 0;
    var stableSeenTicks = Number(
      focusScoreInfo.captureStats && focusScoreInfo.captureStats.consecutiveSeenTicks || 0
    ) || 0;
    var candidateReady = !!analysis.profitCandidateReady;
    var usablePeerNetRatio = Number(profile.peerUsableNetRatio || 0) || 0;
    var highMechanismRate = Number(profile.highMechanismRate || 0) || 0;
    var drainMechanismRate = Number(profile.drainMechanismRate || 0) || 0;
    var desiredModeRate = Number(profile.peerDesiredModeRate || 0) || 0;
    var recoveryText = getPlatformWaterGuardRecoveryText(waterActionGuard, 2);
    var summaryTone = "accent";
    var summaryText = "当前房偏冷";
    if (waterActionGuard.hardBlocked) {
      summaryTone = "danger";
      summaryText = "当前房吃分阻断";
    } else if (waterActionGuard.directFireReady && candidateReady) {
      summaryTone = "good";
      summaryText = "当前房可实战";
    } else if (waterActionGuard.directFireReady || !!profile.positiveStructureReady) {
      summaryTone = "warn";
      summaryText = "当前房观察中";
    }
    var roomStructureText = String(profile.label || "未判断");
    var roomStructureTone = waterActionGuard.hardBlocked
      ? "danger"
      : waterActionGuard.directFireReady
        ? "good"
        : !!profile.positiveStructureReady
          ? "warn"
          : "accent";
    var candidateTone = candidateReady
      ? "good"
      : targetId > 0
        ? "warn"
        : "accent";
    var mechanismTone = drainMechanismRate > 0.08
      ? "danger"
      : highMechanismRate >= 0.05
        ? "good"
        : "accent";
    var peerTone = usablePeerNetRatio > 0 && desiredModeRate >= 0.35
      ? "good"
      : usablePeerNetRatio < 0
        ? "danger"
        : "warn";
    return (
      ...
      '<div style="font-size:12px;font-weight:700;color:#e6edf3;">当前房间实时信号</div>' +
      ...
      buildPanelMetricHtml(
        "机制结构",
        "高机 " +
          formatAmountValue(highMechanismRate * 100) +
          "% / 吃分 " +
          formatAmountValue(drainMechanismRate * 100) +
          "%",
        mechanismTone
      ) +
      buildPanelMetricHtml(
        ...
        "分 " +
          ...
          " / 命中 " +
          formatAmountValue(candidateHitProbability) +
          "% / 稳定 " +
      ...
      buildPanelMetricHtml(
        "同屏结构",
        "净回报 " +
          formatAmountValue(usablePeerNetRatio * 100) +
          "% / mode1 " +
          formatAmountValue(desiredModeRate * 100) +
          "%",
        peerTone
      ) +
      "</div>" +
      '<div style="font-size:11px;color:#98a2b3;margin-top:6px;">只使用当前房间实时结构与当前候选质量，不使用全库时段或目标鱼样本加权。恢复提示: ' +
      escapePanelHtml(recoveryText || "-") +
      "</div>" +
      "</div>"
    );
  }
  ...
  function buildPanelDecisionExplainHtml(status) {
    ...
    var candidateIdentityText = profitCandidate && profitCandidate.node
      ? getCandidateIdentityText(profitCandidate, profitScoreInfo)
      : "暂无利润候选";
    var roomSignalText = waterActionGuard.hardBlocked
      ? "当前房阻断"
      : waterActionGuard.directFireReady
        ? "允许直开"
        : profile.positiveStructureReady
          ? "只够观察"
          : "房态偏冷";
    var roomSignalTone = waterActionGuard.hardBlocked
      ? "danger"
      : waterActionGuard.directFireReady
        ? "good"
        : profile.positiveStructureReady
          ? "warn"
          : "accent";
    var candidateSignalText = candidateReady
      ? "候选达线"
      : profitCandidate && profitCandidate.node
        ? "候选未达线"
        : "暂无候选";
    var candidateSignalTone = candidateReady
      ? "good"
      : profitCandidate && profitCandidate.node
        ? "warn"
        : "accent";
    var actionSignalText = combatPolicy.allowCombat
      ? combatPolicy.probeOnly
        ? "仅可试探"
        : "允许实战"
      : "继续观察";
    var actionSignalTone = combatPolicy.allowCombat
      ? combatPolicy.probeOnly
        ? "warn"
        : "good"
      : "accent";
    ...
    candidateItems.push(
      buildPanelDecisionGateItemHtml(
        ...
        "需要利润候选",
        !!(profitCandidate && profitCandidate.node),
        profitCandidate && profitCandidate.node ? "accent" : "warn"
      )
    );
    candidateItems.push(
      buildPanelDecisionGateItemHtml(
    ...
    actionItems.push(
      buildPanelDecisionGateItemHtml(
        "当前原因",
        getCombatPolicyReasonText(combatPolicy.reason),
        "房态、候选与动作资格共同决定",
        !!combatPolicy.allowCombat,
        actionSignalTone
      )
    ...
    return (
      ...
      "</div>" +
      '<div style="font-size:11px;color:#98a2b3;margin-top:2px;">先看当前房，再看当前候选，最后看当前动作资格；未过项就是当前不给打的直接原因。</div>' +
      '<div style="font-size:11px;color:#98a2b3;margin-top:8px;margin-bottom:6px;">当前房间门槛</div>' +
      '<div style="display:flex;gap:6px;flex-wrap:wrap;">' +
      platformItems.join("") +
      "</div>" +
      '<div style="font-size:11px;color:#98a2b3;margin-top:8px;margin-bottom:6px;">当前候选门槛</div>' +
      '<div style="display:flex;gap:6px;flex-wrap:wrap;">' +
      candidateItems.join("") +
      "</div>" +
      '<div style="font-size:11px;color:#98a2b3;margin-top:8px;margin-bottom:6px;">当前动作资格</div>' +
      '<div style="display:flex;gap:6px;flex-wrap:wrap;">' +
      actionItems.join("") +
      "</div>" +
      "</div>"
  ...
  function buildPanelOverviewHtml(status) {
    ...
    return (
      ...
      buildPanelMetricHtml(
        "吃分机制率",
        formatAmountValue((Number(platformWaterProfile.drainMechanismRate || 0) || 0) * 100) + "%",
        (Number(platformWaterProfile.drainMechanismRate || 0) || 0) > 0.08 ? "danger" : "accent"
      ) +
      buildPanelMetricHtml(
        "高机制率",
        formatAmountValue((Number(platformWaterProfile.highMechanismRate || 0) || 0) * 100) + "%",
        (Number(platformWaterProfile.highMechanismRate || 0) || 0) >= 0.05 ? "good" : "accent"
      ) +
      buildPanelMetricHtml(
        "候选分",
        Number(bestCandidate.score || 0) || 0,
        (Number(bestCandidate.score || 0) || 0) >= (data.config && data.config.minCandidateScore || 0)
          ? "good"
          : "warn"
      ) +

(function () {
  ...
  var PANEL_VERSION = "2026.09.17-monitor-toolbar-v9";
  ...
  function buildCombatPolicy(state, config, analysis) {
    ...
    var peerRecoveryBypassMinCandidateScore = Math.max(
      toNumber(config && config.peerRecoveryBypassMinCandidateScore, 92),
      1
    );
    var peerRecoveryBypassMinHitProbability = Math.max(
      toNumber(config && config.peerRecoveryBypassMinHitProbability, 97),
      1
    );
    var peerRecoveryBypassMinStableSeenTicks = Math.max(
      toInt(config && config.peerRecoveryBypassMinStableSeenTicks, 5),
      1
    );
    var peerRecoveryBypassMinWaterScore = Math.max(
      toNumber(config && config.peerRecoveryBypassMinWaterScore, 50),
      1
    );
    var peerRecoveryBypassMinHighOddRate = clamp(
      toNumber(config && config.peerRecoveryBypassMinHighOddRate, 0.68),
      0.1,
      1
    );
    var peerRecoveryBypassMinHighMechanismRate = clamp(
      toNumber(config && config.peerRecoveryBypassMinHighMechanismRate, 0.18),
      0,
      1
    );
    ...
    var warmingPeerRecoveryBypass = !!(
      ("warming" === level || "open_suspected" === level) &&
      bestCandidate &&
      analysis &&
      analysis.candidateReady &&
      (Number(analysis.bestScore || 0) || 0) >= peerRecoveryBypassMinCandidateScore &&
      (Number(analysis.bestHitProbability || 0) || 0) >= peerRecoveryBypassMinHitProbability &&
      stableSeenTicks >= peerRecoveryBypassMinStableSeenTicks &&
      (Number(profile.score || 0) || 0) >= peerRecoveryBypassMinWaterScore &&
      (Number(profile.highOddRate || 0) || 0) >= peerRecoveryBypassMinHighOddRate &&
      (Number(profile.highMechanismRate || 0) || 0) >= peerRecoveryBypassMinHighMechanismRate
    );
    var policy = {
      allowCombat: false,
    ...
    if (
      ...
      if (warmingPeerRecoveryBypass) {
        policy.allowMediumRamp = true;
        policy.allowHighRamp = false;
        policy.reason = "platform_water_peer_bypass_ready";
      } else {
      policy.allowCombat = false;
      policy.allowMediumRamp = false;
      policy.allowHighRamp = false;
  ...
  function isCandidatePrecisionBlacklisted(config, candidate) {
    ...
    return !!(
      (targetId > 0 && targetId >= minBlacklistedTargetId) ||
  ...
  function getPrecisionTargetTier(config, targetId) {
    ...
    if (normalizedTargetId >= minBlacklistedTargetId) {
      return "blacklist";
    }
  ...
  function canAllowWarmingEmptyOddsPrecision(candidate, scoreInfo, state, config) {
    ...
    var strongHotWindowReady = !!(
      ...
      Number(info.captureStats && info.captureStats.consecutiveSeenTicks || 0) >=
        Math.max(toInt(config.warmingEmptyOddsStrongHotMinStableSeenTicks, 18), 1) &&
      Number(profile.score || 0) >=
        Math.max(toNumber(config.warmingEmptyOddsStrongHotMinWaterScore, 58), 1) &&
      Number(profile.highOddRate || 0) >=
        clamp(toNumber(config.warmingEmptyOddsStrongHotMinHighOddRate, 0.58), 0.1, 1) &&
  ...
  function canPostHitFollowupRampFire(roomStatus, analysis, state, config, combatPolicy) {
    ...
    var profile = pickObject(state && state.platformWaterProfile);
    var bestCandidate = pickObject(analysis && analysis.bestCandidate);
    ...
    if (
      ...
      "waiting_self_recovery" === String(combatPolicy.reason || "")
    ) {
      return false;
    }
    return true;
  ...
  function getIdleKeepaliveStatus(roomStatus, state, config) {
    if (!roomStatus || !roomStatus.inRoom) {
      return {
        ready: false,
  ...
  function fireIdleKeepaliveShot(roomStatus, analysis, state, config, betRuntime) {
    var idleStatus = getIdleKeepaliveStatus(roomStatus, state, config);
    if (!idleStatus.ready) {
      return {
        ok: false,
        fired: false,
    ...
    if (
      ...
      attackProxy.strategy.touchBegin(candidate.node);
      firedViaStrategy = true;
    } else {
      "function" == typeof attackProxy.enableShootTimer &&
        attackProxy.enableShootTimer(false);
    ...
    if (candidate && "function" == typeof attackProxy.setLockedTarget) {
      attackProxy.setLockedTarget(
        Number(candidate.tableEntityID || candidate.symbolID || 0) || 0,
        Number(candidate.sn || 0) || 0
      );
    } else if (!candidate && "function" == typeof attackProxy.cleanLockedTarget) {
      attackProxy.cleanLockedTarget();
    }
    state.lastFireAt = now;
    ...
    if (candidate) {
      registerPendingFireOutcome(state, candidate, now, { fireMode: "idle_keepalive" });
    }
    return {
      ok: true,
      fired: true,
      ...
      idleStatus: idleStatus,
      fireMethod:
        candidate && firedViaStrategy
          ? "strategy_touch_begin"
          : candidate
            ? "direct_manual_fire"
  ...
  function createState() {
    ...
    return {
      ...
      config: {
        ...
        idleKeepaliveRequireReadySeconds: 15,
        ...
        allowColdRoomValidationProbe: true,
        coldRoomValidationObserveSeconds: 18,
        ...
        peerRecoveryBypassMinWaterScore: 50,
        peerRecoveryBypassMinHighOddRate: 0.68,
        peerRecoveryBypassMinHighMechanismRate: 0.18,
        postHitMaxRampStepsWithoutHighConfirmed: 0,
        postHitMaxRampStepsWithHighConfirmed: 1,
        postHitRampMinProfitRaw: 0.6,
        postHitRampMinHitCount: 1,
        ...
        warmingEmptyOddsMinHighMechanismRate: 0.12,
        ...
        warmingEmptyOddsWaitedMinHighMechanismRate: 0.08,
        warmingEmptyOddsStrongHotMinCandidateScore: 94,
        warmingEmptyOddsStrongHotMinHitProbability: 98,
        warmingEmptyOddsStrongHotMinStableSeenTicks: 18,
        warmingEmptyOddsStrongHotMinWaterScore: 58,
  ...
  function tickOnce() {
    ...
    if (
      ...
      var idleKeepaliveResult = fireIdleKeepaliveShot(
        roomStatus,
        analysis,
        runtimeState,
        runtimeState.config,
      ...
      if (
        ...
        return rememberDecision({
          ...
          currentPaolevel: desired.currentPaolevel,
          desiredPaolevel:
            Number(idleKeepaliveResult.desiredPaolevel || desired.desiredPaolevel) || 0,
          betResult: idleKeepaliveResult.betResult || betResult,
          fireResult: idleKeepaliveResult,
          ts: Date.now(),
        });
      }
  ...
  window[GLOBAL_KEY] = {
    ...
    version: "2026.09.17-standalone-native-offline-auto-fish-hotwindow-followup-v2",
  };
